...
void filtrareMasini_UI(MasinaRepository *masinaRepository) 
{
    ...
    if (strcmp(criteriu, "categorie") == 0) 
    {
        ...
        Masina **listaMasiniDeCautat = listaMasiniDupaCategorie(masinaRepository, &dimensiuneListaMasini, filtru);

        //Afisare masini
        for (int i = 0; i < dimensiuneListaMasini; i++)
            printf("Numar inmatriculare: %s | Model: %s | Categorie: %s | Inchiriata: %s\n", listaMasiniDeCautat[i]->numarInmatriculare, listaMasiniDeCautat[i]->model, listaMasiniDeCautat[i]->categorie,
                   listaMasiniDeCautat[i]->inchiriata==0? "False" : "True");
    ...
    else if (strcmp(criteriu, "model") == 0)
    {
        ...
        printf("Rezultate:\n");
        int dimensiuneListaMasini = 0;
        Masina **listaMasiniDeCautat = listaMasiniDupaModel(masinaRepository, &dimensiuneListaMasini, filtru);
...
void afisareMasini_UI(MasinaRepository *masinaRepository) 
{
    for (int i = 0; i < masinaRepository->dimensiune; i++) 
        printf("Numar inmatriculare: %s | Model: %s | Categorie: %s | Inchiriata: %s\n", masinaRepository->listaMasini[i]->numarInmatriculare,
               masinaRepository->listaMasini[i]->model, masinaRepository->listaMasini[i]->categorie, masinaRepository->listaMasini[i]->inchiriata == 0? "False" : "True");
}
...
void afisareMasiniSortate_UI(MasinaRepository *masinaRepository) 
{
    ...
    if (strcmp(criteriuSortare, "model") == 0)
    {
        ...
        if (strcmp(ordineDeSortare, "crescator") == 0)
        {
            //Sortare dupa model, crescator
            printf("Rezultat: \n");
            Masina **listaSortataMasini = sortareMasiniDupaModel(masinaRepository, 1);
            for (int i = 0; i < masinaRepository->dimensiune; i++)
                printf("Numar inmatriculare: %s | Model: %s | Categorie: %s | Inchiriata: %s\n", listaSortataMasini[i]->numarInmatriculare,
                       listaSortataMasini[i]->model, listaSortataMasini[i]->categorie, listaSortataMasini[i]->inchiriata == 0? "False" : "True");

            free(listaSortataMasini);
        } 
        else if (strcmp(ordineDeSortare, "descrescator") == 0)
        {
            //Sortare dupa model, descrescator
            printf("Rezultat: \n");
            Masina **listaSortataMasini = sortareMasiniDupaModel(masinaRepository, 0);
            for (int i = 0; i < masinaRepository->dimensiune; i++)
                printf("Numar inmatriculare: %s | Model: %s | Categorie: %s | Inchiriata: %s\n", listaSortataMasini[i]->numarInmatriculare,
                       listaSortataMasini[i]->model, listaSortataMasini[i]->categorie, listaSortataMasini[i]->inchiriata == 0? "False" : "True");
    ...
    else if (strcmp(criteriuSortare, "categorie") == 0)
    {
        ...
        if (strcmp(ordineDeSortare, "crescator") == 0)
        {
            ...
            printf("Rezultat: \n");
            Masina **listaSortataMasini = sortareMasiniDupaCategorie(masinaRepository, 1);
            for (int i = 0; i < masinaRepository->dimensiune; i++)
                printf("Numar inmatriculare: %s | Model: %s | Categorie: %s | Inchiriata: %s\n", listaSortataMasini[i]->numarInmatriculare,
                       listaSortataMasini[i]->model, listaSortataMasini[i]->categorie, listaSortataMasini[i]->inchiriata == 0? "False" : "True");
        ...
        else if (strcmp(ordineDeSortare, "descrescator") == 0)
        {
            ...
            printf("Rezultat: \n");
            Masina **listaSortataMasini = sortareMasiniDupaCategorie(masinaRepository, 0);
            for (int i = 0; i < masinaRepository->dimensiune; i++) 
                printf("Numar inmatriculare: %s | Model: %s | Categorie: %s | Inchiriata: %s\n", listaSortataMasini[i]->numarInmatriculare,
                       listaSortataMasini[i]->model, listaSortataMasini[i]->categorie, listaSortataMasini[i]->inchiriata == 0? "False" : "True");
            free(listaSortataMasini);
...
void runConsoleApplication() 
{
    ...
    while (rulareProgram) 
    {
        ...
        if (comandaDeExecutat == 0) 
        {
            ...
            //Adauga masina
            adaugareMasina_UI(masinaRepository);
        else if (comandaDeExecutat == 2)
            //Modifica masina existenta
            modificareMasina_UI(masinaRepository);
        else if (comandaDeExecutat == 3)
            //Inchiriere masina
            inchiriereMasina_UI(masinaRepository);
        else if (comandaDeExecutat == 4)
            //Returnare masina
            returnareMasina_UI(masinaRepository);
        else if (comandaDeExecutat == 5)
            //Filtrare masini
            filtrareMasini_UI(masinaRepository);
        else if (comandaDeExecutat == 6)
            //Sortare masini
            afisareMasiniSortate_UI(masinaRepository);
        else if (comandaDeExecutat == 7) 
            afisareMasini_UI(masinaRepository);
    }
}
